...
for classifier in clfs:
   ...
   # if pre_method == '2':
    #    pipeline.set_params(TfidfTransformer(use_idf = False))
...
for classifier in clfs:
    ...
    if (isinstance(classifier, LinearSVC)):
        ...
        cv_grid = GridSearchCV(pipeline, param_grid={
            'clf__learning_rate': np.linspace(0,1,20),#0.5
            'clf__random_state': [42],
        ...
        cv_grid = GridSearchCV(pipeline, param_grid={
            ...
            'clf__random_state': [42],
            'clf_max_depth': [10, 20, 40, 60, 80, 100, None] #try it later
        }, cv=10)
